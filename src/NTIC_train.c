...
void traininit( struct train_data *data , struct net N ){
    memtrack( data->IN=calloc( data->samples , sizeof( float * ) ) );
    memtrack( data->results=calloc( data->samples , sizeof( float * ) ) );
    for( unsigned int i= 0 ; i < data->samples ; i++ ){
        memtrack( data->IN[i]= calloc( N.inputs , sizeof( float ) ) );
        memtrack( data->results[i]= calloc( N.neurons[N.layers - 1] , sizeof( float ) ) );
    }
}
/**
 * @fn train
...
int train( struct net *Net , struct train_data data ){
    ...
    do{
        err_total= 0;
        for( unsigned int i= 0, j, k, l ; i < data.samples ; i++ ){
            memcpy( IN , data.IN[i] , inputs_size );
            run_net( Net );
            for( j= 0 ; j < Net->neurons[prev_layer] ; j++ ){
                err_total+= fabs( delta[j]= data.results[i][j] - *Net->OUT[j] );
                delta[j]*= activation[Net->N[prev_layer][j].FUNC][1]( weighing( Net->N[prev_layer][j]));
            }
            if( err_total < data.tolerance ) continue;
            for( int j= ( int )penu_layer ; j >= 0 ; j-- ){
                ...
                for( k= 0 ; k < Net->neurons[next_layer] ; k++ ){
                    for( l= 0 ; l < Net->N[next_layer][k].inputs ; l++ ) Net->N[next_layer][k].W[l]+= delta[k] * data.learning_rate * *Net->N[next_layer][k].IN[l];
                    Net->N[next_layer][k].B+= delta[k] * data.learning_rate;
                }
                memcpy( delta , delta_h , max_mem );
            }
            for( j= 0 ; j < Net->neurons[0] ; j++ ){
                for( k= 0 ; k < Net->N[0][j].inputs ; k++ ) Net->N[0][j].W[k]+= delta[j] * data.learning_rate * *Net->N[0][j].IN[k];
                Net->N[0][j].B+= delta[j] * data.learning_rate;
            }
        }
    } while( ++attempt < data.max_attempts && err_total > data.tolerance );
    free( delta );
    free( delta_h );
    free( IN );
    return attempt;
